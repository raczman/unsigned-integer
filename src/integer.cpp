...
private:
	explicit Integer(uint64_t _value) : node::ObjectWrap(), value(_value) {}
	...
	NODE_METHOD(FromBits) {
		...
		ReturnNew(info, (uint64_t)((((uint64_t)((uint32_t)high)) << 32) | (uint32_t)low));
	}
	...
	NODE_METHOD(Divide) { UseValue; UseArgument;
		...
		if ((value / arg) > value) return ThrowRangeError(info, "Integer overflow");
		ReturnNew(info, value / arg);
	}
	...
	NODE_METHOD(ShiftLeft) { UseValue;
		...
		ReturnNew(info, (uint64_t)((uint64_t)value << (shift & 63)));
	}
	...
	NODE_METHOD(ValueOf) { UseValue;
		if (value <= MAX_SAFE ) return Return(info, (double)value);
		char buffer[STRING_BUFFER_LENGTH];
	...
	static inline void ReturnNew(NODE_ARGUMENTS info, uint64_t value) {
	...
	static Result Cast(v8::Local<v8::String> string, uint8_t radix) {
		...
		if(is_negative) return Result("The given string contains a negative number");
	...
	static char* WriteString(char* buffer, uint64_t value, uint8_t radix) {
	...
	static const uint64_t MAX_VALUE = 0xffffffffffffffffLL;
	static const uint64_t MIN_VALUE = 0x0000000000000000LL;
	static const uint64_t MAX_SAFE = 9007199254740991LL;
	static const uint64_t MIN_SAFE = -9007199254740991LL;
	static constexpr double MAX_SAFE_DOUBLE = (double)9007199254740991LL;
	static constexpr double MIN_SAFE_DOUBLE = (double)-9007199254740991LL;
